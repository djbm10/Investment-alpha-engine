...
def diagnose_monthly_performance(
    ...
    if latest_summary_type == "phase5" and run_id is None:
        backtest_result, resolved_run_id = _load_phase5_backtest_result(config.paths.processed_dir)
...
def _resolve_latest_summary_type(processed_dir: Path) -> str:
    phase5_summary = processed_dir / "phase5_summary.json"
    ...
    if phase5_summary.exists() and (
        not phase4_summary.exists() or phase5_summary.stat().st_mtime >= phase4_summary.stat().st_mtime
    ) and (
        not phase3_summary.exists() or phase5_summary.stat().st_mtime >= phase3_summary.stat().st_mtime
    ) and (
        not phase2_summary.exists() or phase5_summary.stat().st_mtime >= phase2_summary.stat().st_mtime
    ):
        return "phase5"
...
def _load_phase5_backtest_result(processed_dir: Path) -> tuple[BacktestResult, str]:
    summary_path = processed_dir / "phase5_summary.json"
    if not summary_path.exists():
        raise ValueError("Phase 5 summary was not found for monthly diagnostics.")

    payload = json.loads(summary_path.read_text(encoding="utf-8"))
    run_id = str(payload.get("run_id", "unknown"))
    daily_results = pd.read_csv(processed_dir / "phase5_daily_results.csv", parse_dates=["date"])
    trade_log = pd.read_csv(processed_dir / "phase5_trade_log.csv", parse_dates=["entry_date", "exit_date"])
    monthly_results = pd.read_csv(
        processed_dir / "phase5_monthly_results.csv",
        parse_dates=["test_month", "training_end_date"],
    )
    summary_metrics = {key: value for key, value in payload.items() if key != "config_snapshot"}
    return BacktestResult(
        daily_results=daily_results,
